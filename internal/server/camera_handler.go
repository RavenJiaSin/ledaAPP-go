...
func (s *Server) OpenCamera(c *gin.Context) {
	...
	err := s.Runtime.Camera.Open(req.Name, req.URI)
	if err != nil {
...
func (s *Server) CloseCamera(c *gin.Context) {
	...
	s.Runtime.Camera.Close(req.Name)
...
func (s *Server) CheckCamera(c *gin.Context) {
	...
	_, ok := s.Runtime.Camera.LastFrame(name)
...
func (s *Server) CameraFrame(c *gin.Context) {
	...
	if !ok {
		c.JSON(http.StatusNotFound, gin.H{"error": "frame unavailable"})
		return
	}
...
func encodeJPEG(mat gocv.Mat) ([]byte, error) {
	...
	jpegMat, err := gocv.IMEncode(".jpg", mat)
	if err != nil {
		return nil, err
	}
